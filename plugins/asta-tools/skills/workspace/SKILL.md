...
The scaffold fetches a small Quarto extension (`_extensions/evidence/`) from `asta-plugins` before each render. It follows `main` by default, matching the scaffolded reusable docs workflow, so fixes arrive without copied code drifting; projects that prefer explicit upgrades can set `ASTA_PLUGINS_REF` and pin the workflow to the same ref. The extension lets a factual claim in the prose carry the evidence backing it: the claim gets a subtle dotted underline, and hovering (or keyboard-focusing) it reveals a verbatim quote plus a body-style citation. It renders with pure CSS — so it also survives onto the `what-changed` diff page, where a reviewer can check each claim's backing without leaving the diff.
