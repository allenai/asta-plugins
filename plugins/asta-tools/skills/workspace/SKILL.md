...
- **Bootstrap an empty repo first.** On a brand-new repo with no commits, the *first* branch pushed becomes the repo's default branch — so a feature branch pushed first wrongly becomes the default and leaves no `main` to open a PR against. Before the first save, check whether the repo is initialized: `git ls-remote --heads origin main` (or `gh api repos/{owner}/{repo} -q .default_branch`). If there's no `main`:
  - **Preferred — have the repo created with an initial commit so `main` already exists.** This is the most robust path and works for every agent regardless of harness restrictions on pushing to a default branch. If you're guiding the user, tell them to check **"Add a README file"** (or pick a `.gitignore`/license) when creating the repo in GitHub's web UI, or run `gh repo create <owner>/<name> --add-readme`. With an initial commit present, `main` exists from the start and no seed push is ever needed — you go straight to the feature-branch + PR flow below.
  - **Fallback — seed `main` directly** if the repo already exists empty and can't be recreated: push the initial scaffold commit straight to `main` to establish the default branch — `git push -u origin HEAD:main` — and use that as the base for all later PRs. Seeding `main` is the one legitimate direct push; it's not bypassing review because there's no prior state to review. (Some harnesses restrict direct pushes to a default branch even here — another reason to prefer creating the repo with an initial commit.)

  If `main` already exists, skip both and use the feature-branch flow below.
