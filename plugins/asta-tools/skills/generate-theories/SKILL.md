...
Hand off to the **Asta Artifacts** skill (`asta-tools:artifacts`) to export and index the result — it owns the path convention, manifest, and asta-documents registration. Pass `generate-theories` as the invoking skill and a slug shaped `YYYY-MM-DD-<short-query-slug>` derived from the theory query.
