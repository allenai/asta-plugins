#!/usr/bin/env python3
"""compile-schemas.py — compiler for research-step's assets/workflows.yaml.

Three stages:
  parse  — load workflows.yaml and parse the type mini-DSL and the flow trees.
...
Contract notes encoded here (they mirror the comments in workflows.yaml):
...
GENERATED_NOTE = (
    "generated by scripts/compile-schemas.py from assets/workflows.yaml; do not edit"
)
...
class Compiler:
    ...
    def check_document(self):
        ...
        if not isinstance(d, dict):
            self.err("document", "workflows.yaml is not a mapping")
            return
    ...
    def compile_object(self, fields):
        ...
        # nested fields are always permitted (see the note in workflows.yaml)
        return {
            "type": "object",
            "properties": props,
            "required": required,
...
def main(argv=None):
    ...
    schemas_path = args.skill_dir / "assets" / "workflows.yaml"
    try:
