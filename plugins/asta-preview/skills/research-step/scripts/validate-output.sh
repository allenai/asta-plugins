...
case "$task_type" in
  ...
  replication)                 required="candidate_datasets findings deviations" ;;
  theorize)                    required="theory_query theories" ;;
  *)
    echo "validate-output: unknown task_type '$task_type'" >&2
    echo "validate-output: expected one of scope|definitions|literature_review|hypothesis|experiment_design|evidence_gathering|analysis|synthesis|autods_run|autods_literature_synthesis|replication|theorize" >&2
    exit 3
    ;;
esac
...
case "$task_type" in
  ...
  replication)
    if [[ $(jq '.research_step.output.findings | length' "$file") -gt 0 ]]; then
      jq -e '.research_step.output.findings[0].verdict | IN("replicated","failed_to_replicate","partial","inconclusive","unreachable")' "$file" >/dev/null \
        || { echo "validate-output: findings[].verdict must be one of replicated|failed_to_replicate|partial|inconclusive|unreachable" >&2; exit 4; }
    fi
    ;;
  theorize)
    # No fallback path: theories must be populated by the remote generate-theories agent.
    # If the agent did not return theories, the task should be marked status=failed (and the
    # epic too) and NOT closed; validate-output is not called on a failed task.
    if [[ $(jq '.research_step.output.theories | length' "$file") -eq 0 ]]; then
      echo "validate-output: output.theories must be non-empty; if generate-theories did not return any theories, set status=failed on this task and the epic instead of closing" >&2
      exit 4
    fi
