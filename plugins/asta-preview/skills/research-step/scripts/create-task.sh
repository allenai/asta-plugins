...
# and initialized research_step metadata. output_json stays null until execute
# publishes it via close-task.sh. Prints the new issue id.
set -euo pipefail
...
meta="$(jq -nc --arg f "$flow" --arg tt "$task_type" --argjson inp "$inputs_json" \
  '{research_step: {flow: $f, task_type: $tt, inputs: $inp, output_schema_version: 2, output_json: null}}')"
