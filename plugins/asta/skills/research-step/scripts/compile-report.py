...
def header_includes(short_title):
    """Asta (Ai2) print branding — teal headings/links, green logo + rule, branded
    footer. Serif body font is left untouched (this stays a scientific document)."""
    ...
    lines += [
        # Asta brand palette (design.md/asta): teal primary, green accent, deep teal
        r"\definecolor{astateal}{HTML}{105257}",
        r"\definecolor{astagreen}{HTML}{0FCB8C}",
        r"\definecolor{astadark}{HTML}{032629}",
        r"\definecolor{astapink}{HTML}{E0317F}",  # external-link color (Asta pink, deepened for contrast on white)
        r"\usepackage{sectsty}",
        r"\allsectionsfont{\color{astateal}}",
        # set link colors after hyperref is loaded (avoids \hypersetup-before-load errors).
        # internal refs/TOC = teal; external file/URL links = pink so they clearly read as links.
        r"\AtBeginDocument{\hypersetup{colorlinks=true,linkcolor=astateal,citecolor=astateal,urlcolor=astapink,filecolor=astapink}}",
        ...
        # green brand rule under the running header
        r"\renewcommand{\headrule}{\hbox to\headwidth{\color{astagreen}\leaders\hrule height 1pt\hfill}}",
        r"\pagestyle{fancy}\fancyhf{}",
        r"\fancyhead[L]{\astalogo}",
        r"\fancyhead[R]{\small\itshape\color{astadark}" + short + "}",
        ...
        # title page (plain style) carries the logo + brand rule too
        r"\fancypagestyle{plain}{\fancyhf{}\fancyhead[L]{\astalogo}"
        r"\fancyfoot[R]{\footnotesize\color{gray}\thepage}"
        r"\renewcommand{\headrule}{\hbox to\headwidth{\color{astagreen}\leaders\hrule height 1pt\hfill}}}",
        r"\setlength{\parskip}{0.5\baselineskip}\setlength{\parindent}{0pt}",
    ]
    return lines
...
def flow_diagram(sess, ctx):
    ...
    for i, (title, detail, agent) in enumerate(phases):
        ...
        if agent:
            body += r"\\[2pt]{\scriptsize\itshape\color{astagreen}%s}" % _tex(agent)
    ...
    return (
        "```{=latex}\n"
        "\\begin{center}\n"
        "{\\small\\itshape\\color{astateal}Executed workflow}\\\\[5pt]\n"
        "\\resizebox{\\textwidth}{!}{%\n"
        "\\begin{tikzpicture}[\n"
        "  phase/.style={draw=astateal, line width=0.9pt, rounded corners=4pt, fill=white,"
        " text=astadark, align=center, minimum width=2.6cm, inner sep=7pt, minimum height=1.5cm},\n"
        "  arr/.style={-{Stealth[length=2.4mm]}, draw=astagreen, line width=1.4pt}]\n"
        + "\n".join(nodes + arrows) + "\n"
        "\\end{tikzpicture}}\n"
        "\\end{center}\n"
        "```\n"
