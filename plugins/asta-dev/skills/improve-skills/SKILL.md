...
- **asta-plugins** (always): what it fixes — `Resolves #<issue>` if one exists, else the gap and the behavior it addresses — plus **Validation**: pinned setup (agent version, model, image tag + `@sha256:`), an arms table (baseline, PR, and ablation if you ran one) across cases and metrics, and the transcript read.
  ...
  A results table is only useful if a reader who never saw the run can tell what each number means. A bare `0.667` — or a restated label like "reproducible partial behavior" — is not interpretable on its own. For each case in the table, state (1) **what the case prompts and what its metric scores** — the behavior being checked and what `0` vs `1` represents (a routing metric scores whether the expected skill was invoked; an outcome metric scores a sandbox observable — pull this from the case's metric handler / the [`asta_skills` README](https://github.com/allenai/asta-bench-private/blob/main/astabench/ai2/evals/asta_skills/README.md)), and (2) **why the number came out that way**, grounded in the transcript — what the agent actually did on the passing vs. failing epochs, not just a paraphrase of the score. A mid score (e.g. `0.667` = 2 of 3 epochs) needs the transcript to say what differed between epochs; a `0` needs what the agent did instead. This is the qualitative interpretation the numbers alone can't carry — without it the table is un-actionable for the next session.
