...
allowed-tools: Read(project.md) Read(work/**) Skill(asta-assistant:do-work) Skill(asta-assistant:plan-work) Skill(asta-assistant:brainstorm)
---
...
2. **Pick the next action.**
   - If any item has `status: ready`, pick the first one and hand off to **do-work** with that slug.
...
| `done` | Work and review complete | (skip; move to Completed Work via **save-work**) |
