...
Create or extend the research graph. The workflows live in `assets/schemas.yaml` (`workflows`) — plan reads them, it does not hardcode the sequence. Two modes:
...
How to read a flow node (step vs group vs `child_workflows` composition vs `replan` fan-out) is documented in `assets/README.md`. The build-specific rule: a **fan-out group** (`replication`, `verification`, `testing`) inserts **one branch level per item** — the group node, then one branch epic per item, then the group's steps repeated under each branch; the group `mission` names what to branch on.
...
- When `novelty_assessment` closes: create a `verification` branch only per theory in `testable_theory_ids`; the rest are left as recorded (signal basis `untestable`) and simply not verified.
...
- A fan-out group: `bd create --parent <group> -t epic` one branch epic per item, then the branch steps under each via `create-task.sh` — **but a gated group lays only the steps up to its gate**: under a `replication` or `testing` branch create only `experiment_design`; the Gate below creates the rest when it closes. Ungated branches (`verification`: analysis, audit) get all their steps at branch creation. Record why for any branch the data can't support, rather than skipping it.
