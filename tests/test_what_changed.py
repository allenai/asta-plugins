...
def test_client_folder_guards_tables_lists_and_inline_change_wrappers():
    """The client-side folder must not corrupt structured or inline content.

    Regression for a diff whose folder (a) recursed into <ins>/<del> wrappers,
    where every child looks unchanged (it has no ins/del *descendant*), and so
    folded genuinely-inserted inline content — e.g. collapsing an inline hover
    popover's quote behind a bogus "N unchanged blocks" toggle; (b) inserted its
    fold disclosure between a table's rows or a list's items, which is invalid and
    visibly breaks the table/list; and (c) descended into absolutely-positioned /
    hidden subtrees (tooltips, asides) that are not part of the readable flow.

    Folding is client-side progressive enhancement, so this asserts the guards are
    present in the shipped script (the suite can't execute the DOM folding).
    """
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        new.mkdir()
        shell = "<html><head></head><body><main>{b}</main></body></html>"
        (old / "index.html").write_text(shell.format(b="<p>base</p>"))
        (new / "index.html").write_text(
            shell.format(b="<p>base</p><p>added paragraph of content</p>")
        )
        result = WHAT_CHANGED.build(old, new, "", "PR #9")

    # (a) never recurse into an <ins>/<del> change wrapper.
    assert 'el.tagName !== "INS" && el.tagName !== "DEL"' in result
    # (b) never insert a fold disclosure as a direct child of a table row/section
    # or a list — the fold guard names those parents and returns early.
    assert "NO_FOLD_CHILDREN" in result
    assert "if (NO_FOLD_CHILDREN[container.tagName]) return;" in result
    for parent in ("TABLE", "TBODY", "TR", "UL", "OL"):
        assert parent in result
    # (c) out-of-flow / hidden subtrees are skipped, so an absolutely-positioned
    # hover popover's content is never folded.
    assert "function inFlow(el)" in result
    assert 's.position === "absolute"' in result
    assert 's.visibility === "hidden"' in result
